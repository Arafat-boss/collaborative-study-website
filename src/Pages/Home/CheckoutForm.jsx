...
const CheckoutForm = ({ booked = {} }) => {
  ...
  if (bookingSuccessData) {
    return (
      <div className="bg-white dark:bg-slate-900 rounded-[5px] border border-gray-100 dark:border-slate-800 shadow-sm p-6 sm:p-10 max-w-xl mx-auto text-center space-y-6 transition-colors">
        ...
        {/* Invoice Info Card */}
        <div className="p-4 rounded-[5px] bg-slate-50 dark:bg-slate-800/60 border border-gray-100 dark:border-slate-800 text-left space-y-2 text-xs">
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-2">
          <button
            ...
            className="flex-1 py-3 px-5 rounded-[5px] bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs flex items-center justify-center gap-2 shadow-sm transition-all"
          >
            <LuFileText className="text-base text-white" />
            <span>Download / Print Invoice</span>
          </button>
          <Link
            to="/dashboard/viewBookedSession"
            className="flex-1 py-3 px-5 rounded-[5px] bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-gray-800 dark:text-slate-200 font-semibold text-xs flex items-center justify-center gap-2 transition-colors"
          >
            <span>My Booked Sessions</span>
            <LuArrowRight className="text-blue-600 dark:text-blue-400" />
          </Link>
        </div>
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-[5px] border border-gray-100 dark:border-slate-800 shadow-sm p-6 sm:p-8 md:p-10 max-w-xl mx-auto space-y-6 transition-colors">
      
      {/* Session Header Card */}
      <div className="flex flex-col sm:flex-row items-center gap-4 p-4 rounded-[5px] bg-slate-50 dark:bg-slate-800/60 border border-gray-100 dark:border-slate-800">
        <img
          src={booked.sessionImage || "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=200"}
          alt={booked.sessionTitle || "Session"}
          className="w-full sm:w-20 h-28 sm:h-20 rounded-[5px] object-cover shadow-sm flex-shrink-0"
        />
        <div className="min-w-0 flex-1 text-center sm:text-left space-y-1">
          <span className="text-[10px] font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/60 px-2 py-0.5 rounded-[5px] border border-blue-100 dark:border-blue-900/50">
          ...
          <p className="text-xs text-gray-500 dark:text-slate-400 flex items-center justify-center sm:justify-start gap-1">
            <LuUser className="text-blue-600 dark:text-blue-400" /> By {booked.tutorName || "Instructor"}
          </p>
        </div>
      </div>
      ...
      {transactionId && (
        <div className="p-4 rounded-[5px] bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800 text-xs text-blue-800 dark:text-blue-300 font-mono">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        {registrationFee > 0 && (
          <div className="space-y-2">
            ...
            <div className="p-4 border border-gray-200 dark:border-slate-700 rounded-[5px] bg-slate-50 dark:bg-slate-800 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
              <CardElement
                options={{
                  style: {
                    base: {
        ...
        <button
          ...
          className="w-full py-3.5 px-6 rounded-[5px] bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white font-semibold text-sm sm:text-base shadow-sm transition-all duration-200 flex items-center justify-center gap-2"
        >
