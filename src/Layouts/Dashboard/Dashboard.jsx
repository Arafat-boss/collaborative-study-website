    ...
    <div className="relative min-h-screen bg-slate-50 dark:bg-[#080d1a] text-gray-800 dark:text-slate-100 flex flex-col lg:flex-row antialiased transition-colors duration-300">
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white/90 dark:bg-[#0c1222]/90 backdrop-blur-md border-b border-gray-200 dark:border-slate-800 px-4 py-3 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 rounded-[5px] text-gray-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-slate-800 focus:outline-none transition-colors"
            aria-label="Open sidebar menu"
          >
            <HiMenuAlt2 className="text-2xl text-blue-600 dark:text-blue-400" />
          </button>
          <Link to="/" className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-[5px] bg-blue-600 flex items-center justify-center text-white">
              <LuGraduationCap className="text-lg" />
            </div>
            <span className="font-bold text-gray-900 dark:text-white text-sm sm:text-base">Dashboard</span>
          </Link>
        </div>

        <div className="flex items-center gap-2.5">
          <ThemeToggle className="w-8 h-8 p-0 flex items-center justify-center rounded-[5px]" />
          <span className={`text-[11px] font-bold px-2.5 py-0.5 rounded-[5px] border ${roleBadgeColor}`}>
            {role || "User"}
          </span>
          <img
            ...
            alt="User Avatar"
            className="w-8 h-8 rounded-[5px] border border-gray-300 dark:border-slate-700 object-cover"
          />
        </div>
      </div>
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          {/* Drawer Content */}
          <aside className="relative flex-1 flex flex-col max-w-xs w-full bg-white dark:bg-[#0c1222] shadow-xl p-6 overflow-y-auto border-r border-gray-200 dark:border-slate-800">
            <div className="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-slate-800">
              <Link to="/" onClick={closeSidebar} className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-[5px] bg-blue-600 flex items-center justify-center text-white">
                  <LuGraduationCap className="text-lg" />
                </div>
                <span className="font-bold text-gray-900 dark:text-white text-base">Collaborative Study</span>
              </Link>
              <button
                onClick={closeSidebar}
                className="p-1.5 rounded-[5px] text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-slate-800"
              >
            ...
            <div className="flex items-center gap-3 py-4 my-2 border-b border-gray-100 dark:border-slate-800">
              <img
                ...
                src={user?.photoURL || "https://placehold.co/100x100?text=User"}
                alt="User Avatar"
                className="w-12 h-12 rounded-[5px] border border-blue-500 object-cover flex-shrink-0"
              />
              <div className="flex flex-col min-w-0">
                ...
                <span className={`inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-[5px] border w-max ${roleBadgeColor}`}>
                  {roleFormatted}
                </span>
              </div>
            </div>
      ...
      <aside className="hidden lg:flex flex-col w-72 min-h-screen bg-white dark:bg-[#0c1222] border-r border-gray-200 dark:border-slate-800/80 p-5 flex-shrink-0 sticky top-0 h-screen overflow-y-auto z-20 transition-colors">
        ...
        <div className="flex items-center justify-between pb-5 border-b border-gray-100 dark:border-slate-800">
          <Link to="/" className="flex items-center gap-2.5 min-w-0 flex-1">
            <div className="w-10 h-10 rounded-[5px] bg-blue-600 flex items-center justify-center text-white shadow-sm flex-shrink-0">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[10px] uppercase font-bold tracking-wider text-gray-400 dark:text-slate-500 mt-0.5">
                Dashboard Portal
              </span>
            </div>
          </Link>
          <div className="flex-shrink-0 ml-2">
            <ThemeToggle className="w-9 h-9 p-0 flex items-center justify-center rounded-[5px] bg-slate-100 dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/80 shadow-sm" />
          </div>
        </div>

        {/* User Card */}
        <div className="my-6 p-4 rounded-[5px] bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 flex flex-col items-center text-center">
          <img
            referrerPolicy="no-referrer"
            src={user?.photoURL || "https://placehold.co/100x100?text=User"}
            alt="User Avatar"
            className="w-16 h-16 rounded-[5px] border border-blue-500/40 shadow-sm object-cover mb-3"
          />
          ...
          <span className={`inline-block mt-2.5 text-xs font-bold px-3 py-1 rounded-[5px] border ${roleBadgeColor}`}>
            {roleFormatted}
          </span>
        </div>
